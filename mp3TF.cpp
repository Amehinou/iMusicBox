...
//
void mp3TF::checkStatic() {
  static uint8_t play_cmd [10] = { 0X7E, 0xFF, 0x06, 0X42, 01, 00, 00, 0xFE, 0xee, 0XEF};
  sendCmd (play_cmd);
}
...
void mp3TF::setVolume(uint16_t index) {
  static uint8_t play_cmd [10] = { 0X7E, 0xFF, 0x06, 0X06, 00, 00, 00, 0xFE, 0xee, 0XEF};
  play_cmd[5] = (uint8_t)(index >> 8);
  play_cmd[6] = (uint8_t)(index);
  sendCmd (play_cmd);
}
...
void mp3TF::playWithSakura(uint16_t index) {
  static uint8_t play_cmd [10] = { 0X7E, 0xFF, 0x06, 0X13, 00, 00, 00, 0xFE, 0xee, 0XEF};
  play_cmd[5] = (uint8_t)(index >> 8);
  play_cmd[6] = (uint8_t)(index);
  sendCmd (play_cmd);
}
...
//
void mp3TF::loopAllon() {
  static uint8_t loopAllon_cmd [] = { 0X7E, 0xFF, 0x06, 0X11, 00, 11, 11, 0xFE, 0xF9, 0XEF};
  sendCmd (loopAllon_cmd);
}